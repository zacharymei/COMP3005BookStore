...
function submitOrder(){
	...
	info.subtotal = Number(currentSubtotal).toFixed(2);
	info.total = Number(currentTotal).toFixed(2);
	info.fee = Number(currentFee).toFixed(2);
	info.tax = Number(currentSubtotal * 0.1).toFixed(2);
	info.order = order;
...
function getMenuHTML(rest){
	...
	Object.keys(menu).forEach(key =>{
		...
		Object.keys(menu[key]).forEach(id => {
			item = menu[key][id];
			result += `${item.name} (\$${item.price}) <img src='../images/add.jpg' style='height:20px;vertical-align:bottom;' onclick='addItem(${id})'/> <br>`;
...
function updateOrder(){
	...
	Object.keys(order).forEach(id =>{
		...
		result += `${item.name} x ${order[id].quantity} (${(item.price * order[id].quantity).toFixed(2)}) <img src='../images/remove.jpg' style='height:15px;vertical-align:bottom;' onclick='removeItem(${id})'/><br>`;
	});
